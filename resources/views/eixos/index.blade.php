    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6">
                ...
                @if ($eixos->isEmpty())
                    <p class="text-gray-600 dark:text-gray-300">Nenhum eixo cadastrado.</p>
                @else
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            @foreach ($eixos as $eixo)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <td class="px-6 py-4 text-gray-900 dark:text-gray-100">{{ $eixo->nome }}</td>
                                    <td class="px-6 py-4">
                                        <a href="{{ route('eixos.show', $eixo) }}"
                                           class="inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">
                                            Ver
                                        </a>
                                    </td>
                                    <td class="px-6 py-4">
                                        <a href="{{ route('eixos.edit', $eixo) }}"
                                           class="inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">
                                            Editar
                                        </a>
                                    </td>
                                    <td class="px-6 py-4">
                                        <form action="{{ route('eixos.destroy', $eixo->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Tem certeza que deseja remover este eixo?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
