...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('declaracaos.update', $declaracao->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-3">
                <label for="hash" class="form-label">Hash</label>
                <input type="text" name="hash" id="hash" class="form-control" value="{{ old('hash', $declaracao->hash) }}" required>
            </div>

            <div class="mb-3">
                <label for="data" class="form-label">Data</label>
                <input type="date" name="data" id="data" class="form-control" value="{{ old('data', $declaracao->data) }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="comprovante_id" id="comprovante_id" class="form-select">
                    @foreach ($comprovantes as $comprovante)
                        <option value="{{ $comprovante->id }}" {{ $declaracao->comprovante_id == $comprovante->id ? 'selected' : '' }}>{{ $comprovante->nome }}</option>
                    @endforeach
                </select>
            </div>
